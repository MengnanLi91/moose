...
// matrix-free application of a mass operator. Arbitrary mesh and solution degrees in 1D, 2D and 3D
// are supported from the same code.
//
// The example has no dependencies, and is designed to be self-contained.
// For additional examples that use external discretization libraries (MFEM, PETSc, etc.) see the
// subdirectories in libceed/examples.
//
// All libCEED objects use a Ceed device object constructed based on a command line argument
// (-ceed).
//
// Build with:
//
...
#include "LibCEEDLibLinkTest.h"

#include <ceed.h>
#include <math.h>

registerMooseObject("MooseTestApp", LibCEEDLibLinkTest);

InputParameters
LibCEEDLibLinkTest::validParams()
{
...
LibCEEDLibLinkTest::LibCEEDLibLinkTest(const InputParameters & params)
  : GeneralVectorPostprocessor(params)
{
  ...
  CeedInt test = 0, gallery = 0;

  if (prob_size < 0)
    prob_size = test ? 8 * 16 : 256 * 1024;
  ...
  CeedBasisCreateTensorH1Lagrange(
      ceed, dim, num_comp_x, mesh_degree + 1, num_qpts, CEED_GAUSS, &mesh_basis);
  ...
  if (!test)
  {
    ...
    if (dim > 1)
      printf(", ny = %" CeedInt_FMT, num_xyz[1]);
    if (dim > 2)
      printf(", nz = %" CeedInt_FMT, num_xyz[2]);
    printf("\n");
    // LCOV_EXCL_STOP
  }
  ...
  BuildCartesianRestriction(
      ceed, dim, num_xyz, mesh_degree, num_comp_x, &mesh_size, num_qpts, &mesh_restriction, NULL);
  BuildCartesianRestriction(ceed,
                            dim,
                            num_xyz,
                            sol_degree,
                            1,
  ...
  CeedOperatorSetField(
      op_build, "weights", CEED_ELEMRESTRICTION_NONE, mesh_basis, CEED_VECTOR_NONE);
  ...
  else
  {
    ...
    if (fabs(volume - exact_volume) > tol)
...
int
LibCEEDLibLinkTest::GetCartesianMeshSize(CeedInt dim, CeedInt degree, CeedInt prob_size, CeedInt num_xyz[dim])
{
...
int
LibCEEDLibLinkTest::BuildCartesianRestriction(Ceed ceed,
                          CeedInt dim,
                          CeedInt num_xyz[dim],
                          CeedInt degree,
                          CeedInt num_comp,
                          ...
                          CeedElemRestriction * q_data_restriction)
{
  ...
  *size = scalar_size * num_comp;
  ...
  CeedInt * elem_nodes = static_cast<CeedInt *>(malloc(sizeof(CeedInt) * num_elem * num_nodes));
...
int
LibCEEDLibLinkTest::SetCartesianMeshCoords(CeedInt dim,
                       CeedInt num_xyz[dim],
                       CeedInt mesh_degree,
                       CeedVector mesh_coords)
{
...
CeedScalar
LibCEEDLibLinkTest::TransformMeshCoords(CeedInt dim, CeedInt mesh_size, CeedVector mesh_coords)
{
